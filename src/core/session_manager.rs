...
impl SessionManager {
    ...
    pub fn get_or_create_session(&self, id: &str, intent: Option<&str>) -> Arc<RwLock<Session>> {
        {
        ...
        let mut session = Session::new(new_id.clone());
        if let Some(intent) = intent {
            session = session.with_intent(intent);
        }
        ...
        self.sessions.write().unwrap()
            .insert(new_id.clone(), session_arc.clone());
        session_arc
    }
